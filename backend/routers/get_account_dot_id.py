...
router = APIRouter(
    tags=["Get account_dot_id"],
    prefix="/api"
)


@router.get("/accounts/user/dot/{user_id}", response_model=dict)
async def get_accountid_by_user_id(user_id: str, db: Session = Depends(get_db)):
