...
class SparkMLService:
    ...
    def classify_transaction(self, transaction: Dict[str, Any]) -> str:
        ...
        try:
            ...
            # Create DataFrame
            df = self.spark.createDataFrame(features, schema=transaction_schema)
            
            # Make prediction
            prediction = self.classification_model.transform(df)
            
            # Get predicted category
            category = prediction.select("prediction").first()[0]
            
            logger.info(f"Classified transaction {transaction['remittance_information']} as {category}")
            return category
            
        except Exception as e:
            logger.error(f"Error classifying transaction {transaction['reference']}: {str(e)}")
            raise
    ...
    def predict_user_spending(self, user_id: str, transactions: List[Dict[str, Any]], 
                            target_period_days: int = 30) -> Dict[str, Any]:
        """Predict user's spending for the next period."""
        try:
            # Calculate target period dates
            prediction_date = datetime.now().date()
            target_period_start = prediction_date
            target_period_end = prediction_date + timedelta(days=target_period_days)
            
            # Prepare features for prediction
            # This would include historical spending patterns, category distributions, etc.
            features = {
                "user_id": user_id,
                "historical_spending": sum(t["amount"] for t in transactions),
            ...
            # Make prediction
            prediction = self.prediction_model.transform(df)
            
            # Get predicted amount
            predicted_amount = float(prediction.select("prediction").first()[0])
            ...
            result = {
                ...
                "confidence": 0.85  # This would come from the model in a real implementation
            }
            
            logger.info(f"Predicted spending for user {user_id}: {predicted_amount}")
            return result
